...
import { Repository, Brackets, getRepository } from 'typeorm';
...
import PasswordReset from '../models/user/PasswordReset';
...
export default class UserBusiness implements IUserBusiness {
    ...
    async forgotPassword(email: string): Promise<boolean> {
        ...
        if (user) {
            const key = randomize('a', 10);
            await this.userRepository.update(user.id, { keyRandom: key });

            MailHelper.sendMail(user.email, 'Reset Password', '', `Please click link and update Password:  ${process.env.FRONTEND_PROTOTYPE + '://' + process.env.FRONTEND_DOMAIN + process.env.FRONTEND_PORT + '/recover-password/' + user.id}?key=${key}`);
            return true;
        }
        else
            return false;
    ...
    async signin(data: UserSignin): Promise<UserAuthentication> {
        ...
        if (data.expiredInMS === undefined)
            data.expiredInMS = 7 * 24 * 60 * 60 * 1000; // 7 days

        if (!user.accessToken || !user.tokenExpire || user.tokenExpire < new Date()) {
            const accessToken = jwt.sign({ user: user }, process.env.SECRETKEY, { expiresIn: '7d' });
    ...
    async resetPassword(id: number, data: PasswordReset): Promise<boolean> {
        ...
        const user = await this.userRepository.createQueryBuilder('user')
            .whereInIds([id])
            .andWhere('user.deletedAt IS NULL')
            .andWhere('user.keyRandom = :keyRandom', { keyRandom: data.keyRandom })
            .getOne();
        ...
        await this.userRepository.update(id, { password: this.hashPassword(data.newPassword), keyRandom: '' });
        return true;
    }
